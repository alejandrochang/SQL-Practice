...
def population_constraint
  ...
  WHERE
    population BETWEEN
      (SELECT population FROM countries WHERE name = 'Canada') + 1
      AND
      (SELECT population FROM countries WHERE name = 'Poland') - 1
  SQL
end
...
def sparse_continents
  ...
  WHERE
    continent IN (
      SELECT continent FROM (SELECT continent, MAX(population) AS max_pop FROM countries GROUP BY continent) AS temp_db
